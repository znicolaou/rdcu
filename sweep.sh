...
for j in `seq 1 11`; do
	...
	if [ ! -f scratch/3dcgle/${i}/${j}/diffs.dat ]; then 
		echo "./makelines.py --rm 1 --Ns 128 128 128 --Ls 200 200 200 --T 2000 --T1 1900 --dt1 10 --dt 0.1 --b ${b} --c ${c} --filebase scratch/3dcgle/${i}/${j}/ &"
		./makelines.py --rm 1 --Ns 128 128 128 --Ls 200 200 200 --T 2000 --T1 1900 --dt1 10 --dt 0.1 --b ${b} --c ${c} --filebase scratch/3dcgle/${i}/${j}/ &
	fi
	gid=$((gid+1))
	if [ $gid -ge 1 ]; then
		gid=0
